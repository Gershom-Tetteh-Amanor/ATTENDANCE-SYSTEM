/* ============================================
   ...
   FIREBASE MODE  (window._db is set):
     All reads/writes go to Firebase Realtime DB.
     Real-time listeners fire across all devices.

   DEMO MODE  (window._db is null / not configured):
     localStorage is the store.
     BroadcastChannel syncs all open tabs in the
     same browser so student check-ins show live
     on the lecturer tab without needing Firebase.
     1.5 s polling acts as belt-and-braces fallback.
   ============================================ */
'use strict';

const DB = (() => {
  const fb = () => window._db;
  const k  = s => String(s).replace(/[.#$[\]/]/g, '_'); /* Firebase key sanitiser */
  ...
  const fbListen = (p, cb) => {
    const ref = fb().ref(p), fn = s => cb(s.val());
    ref.on('value', fn);
    return () => ref.off('value', fn);
  };

  /* ══ DEMO store (localStorage + BroadcastChannel) ══ */
  ...
  const demoRemove = p      => {
    ...
    delete n[parts[parts.length - 1]]; save(s); bcast(parts[0]);
  };
  const demoPush = (p, v) => {
    const id = 'k' + Date.now().toString(36) + Math.random().toString(36).slice(2, 5);
    demoSet(`${p}/${id}`, { ...v, _k: id }); return id;
  };
  const demoListen = (p, cb) => {
    cb(demoGet(p));
    const top = p.split('/')[0];
    const onMsg = e => { if (e.data?.top === top) cb(demoGet(p)); };
    try { getBC()?.addEventListener('message', onMsg); } catch {}
    const timer = setInterval(() => cb(demoGet(p)), 1500);
    return () => { clearInterval(timer); try { getBC()?.removeEventListener('message', onMsg); } catch {} };
  };

  /* ══ Unified API — auto picks mode ══ */
  ...
  const TA = {
    ...
    setInvite:    (key, d)    => set(`taInvites/${key}`, d),
    updateInvite: (key, d)    => update(`taInvites/${key}`, d),
    inviteByCode: async code  => {
      const all = await get('taInvites');
      if (!all) return null;
      return Object.entries(all).find(([, v]) => v.code === code) || null;
    },
  };
  ...
  const SESSION = {
    ...
    listenRecords: (id, cb) => listen(`sessions/${id}/records`,
      v => cb(v && typeof v === 'object' ? Object.values(v) : [])),
    listenBlocked: (id, cb) => listen(`sessions/${id}/blocked`,
      v => cb(v && typeof v === 'object' ? Object.values(v) : [])),
  };
  ...
  return { SA, CA, LEC, TA, UID, SESSION, BACKUP };
})();
